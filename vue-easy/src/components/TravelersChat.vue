<template>
  <div class="chat-room" v-if="true">
    ...
    <div class="chat-messages">
      <div v-for="{ senderid, msg } in messages" class="message">
        <div v-if="props.travelerId == senderid" class="text-start">{{ msg }}</div>
...
const sendMessage = async () => {
  if (newMessage.value) {
    const API = `${import.meta.env.VITE_API_easystep}/chatTraroom/startChatToTra/${props.travelerId}`
...
const findAllMessage = async () => {
  const API = `${import.meta.env.VITE_API_easystep}/chatTraroom/showchats/${props.travelerId}`
